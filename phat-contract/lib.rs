...
// #[pink::contract(env = xvm_environment::XvmDefaultEnvironment)]
mod phat_contract {
    use super::pink;
...
//     #[derive(Deserialize, Encode, Clone, Debug, PartialEq)]
//     // #[cfg_attr(feature = "std", derive(scale_info::TypeInfo))]
//     pub struct VehicleResponse<'a, 'de> {
//         RPM: Rpm<'a, 'de>,
//         SPEED: Speed<'a, 'de>,
//         COOLANT_TEMP: CoolantTemp<'a, 'de>,
//         THROTTLE_POS: ThrottlePos<'a, 'de>,
//         INTAKE_TEMP: IntakeTemp<'a, 'de>,
...
//     // #[cfg_attr(feature = "std", derive(scale_info::TypeInfo))]
//     pub struct Rpm<'a, 'de> {
//         value: &'a str, // might wrap in a box instead
//         command: Command<'a, 'de>,
//         time: u64,
//         unit: &'a str,
//         phantom: PhantomData<&'de ()>
//     }
...
//     }
    ...
    impl Phatcon {
        ...
        // #[ink(message)]
        // pub fn get_vehicle_reponse(&self) -> Result<Vec<u8>> {
        //     let resp = http_get!(format!(
        //         "https://twenty-bags-wink.loca.lt/",
        //     ));
        ...
        //     let result: VehicleResponse = serde_json_core::from_slice(&resp.body)
        //         .or(Err(Error::InvalidResponseBody))?
        //         .0;
        //     println!("RPM: {:?}", resp.body);
        //     // debug!("RPM: {}", result.RPM.value);
    ...
    mod tests {
        ...
        fn it_works() {
            ...
            let phat_con = Phatcon::new();
            let account = String::from("0xD0fE316B9f01A3b5fd6790F88C2D53739F80B464");
            let res = phat_con.get_eth_balance(account.clone());
            assert!(res.is_ok());
        ...
        //     let phat_con = Phatcon::new();
        //     let res = phat_con.get_vehicle_reponse();
        //     // println!(res.)
        //     assert!(res.is_ok());

        //     println!("Vehicle response: {:?}", res.unwrap());
        // }
    }
}
